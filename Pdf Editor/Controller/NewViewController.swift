...
class NewViewController: UIViewController {
        ...
        Manager.sharedInstance.createNewDocument(fromTemplate: item, completion: { (pdfItem) in
            ...
            if let pdfItem = pdfItem {
                ...
                UIView.animate(withDuration: 0.5, animations: {
                    imageView.frame = CGRect(x: 0, y: 100, width: 0, height: 0)
                }) { (completed) in
                    if completed {
    ...
    @IBAction func onNewFromTemplate(_ sender: SpringButton) {
...
extension NewViewController: ItemListViewControllerDelegate {
